...
def KaprekarsConstant(num):
    i = 1
    while True:
        num = calculate_KaprekarNumber(str(num).zfill(4))
        if num == 6174:
            break
        i += 1
    return i
...
def calculate_KaprekarNumber(strNum):
    descending = int("".join(sorted(strNum, reverse=True)))
    ascending = int("".join(sorted(strNum)))
